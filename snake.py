...
# Set up the font and initialize global variables
pygame.font.init()
FONT = pygame.font.SysFont('Arial', 20)
current_algorithm = None  # Global variable for the algorithm name
...
class cube(object):
    ...
    def __init__(self, start, dirnx=1, dirny=0, color=(27, 247, 236)):
        self.pos = start
        self.dirnx = 1
        self.dirny = 0
        self.color = color
    ...
    def draw(self, surface, eyes=False):
        ...
        radius = dis // 2 - 1  # Adjust radius to fit within grid cells
        ...
        if eyes:
            ...
            eye_offset_y = dis // 4
            # Left eye
            pygame.draw.circle(surface, (255,0,0), (center_x - eye_offset_x, center_y - eye_offset_y), eye_radius)
            # Right eye
            pygame.draw.circle(surface, (255,0,0), (center_x + eye_offset_x, center_y - eye_offset_y), eye_radius)
...
def redrawWindow(surface, s):
    ...
    # Only display algorithm name if current_algorithm is set
    if current_algorithm:
        algorithm_text = FONT.render(f"Algorithm: {current_algorithm}", True, (0, 0, 0))
        surface.blit(algorithm_text, (10, 10))  # Position text at the top lef
...
def bestFirstSearch(s, i, slow):
    ...
    from util import PriorityQueue
    bestFirst_priorityqueue = PriorityQueue()  # priority queue for best first search
    visited = set()
    bestFirst_priorityqueue.push((s.getStartState(), []), manhattanHeuristic(s.getStartState()))

    while not bestFirst_priorityqueue.isEmpty():
        current, directions = bestFirst_priorityqueue.pop()
        ...
        if current not in visited:
            ...
            for childNode, direction, _ in s.getSuccessors(current):
                if childNode not in visited:
                    hCost = manhattanHeuristic(childNode)  # Priority based on heuristic only
                    bestFirst_priorityqueue.push((childNode, directions + [direction]), hCost)

# --------------------------------------------------------------------- End of Best First Search
...
def runMultiple(times):
    ...
    # Calculate averages for each algorithm, including Best First Search (index 4)
    ...
    averageCalcCosts[4] = sum(allCalcCosts[4]) / times  # Best First Search

    # -------------------------------------------------------------- Bar Graph for Average Scores
    data = {
        "Algorithm": ["DFS", "BFS", "ASTAR", "UCS", "Best First Search"],
        "AvgScore": [round(x, 2) for x in averageCalcCosts]
    }
    
    # Load data into a DataFrame and create a bar chart
    dataFrame = pd.DataFrame(data=data)
...
def showExample():
    ...
    for name, algorithm in algorithms:
        current_algorithm = name  # Update the global variable with the current algorithm name
        for i in range(5):
            #redrawWindow(surface, mySnake)  # Call redrawWindow without additional parameters
            algorithm(mySnake, i, goSlow)
        mySnake.reset(START_POS)
